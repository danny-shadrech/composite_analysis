...
for yr in elnino_years:
    ax_nino.axhspan(yr + 0.5, yr + 1.5, color=EL_NINO_COL, alpha=0.10, linewidth=0)
for yr in lanina_years:
    ax_nino.axhspan(yr + 0.5, yr + 1.5, color=LA_NINA_COL, alpha=0.10, linewidth=0)
...
norm = mcolors.Normalize(vmin=0, vmax=0.5)     # actual data range ~0–3, use 0–0.5 to show structure
...
ax_hov.set_title(
    r'Hovmöller — Skipjack Biomass Mean (mt km$^{-2}$) | 10°S–10°N',
...
cbar.set_label('Biomass (mt km⁻²)  [0–0.5]', color='black', fontsize=8)
